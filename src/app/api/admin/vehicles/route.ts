...
export async function GET() {
  ...
  if (session?.user?.role !== "ADMIN" && !session?.user?.canConfigureSystem && !session?.user?.canManageShifts && !session?.user?.isUfficiale) return NextResponse.json({ error: "Unauthorized" }, { status: 401 })
