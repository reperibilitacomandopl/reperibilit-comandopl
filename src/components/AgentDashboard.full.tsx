...
const RAW_AGENDA_CATEGORIES = [
  {
    group: "Ferie e Festivit├á", emoji: "­ƒÅû´©Å", color: "amber",
    items: [
      { code: "0015", label: "Ferie Anno Corrente" },
      { code: "0016", label: "Ferie Anni Precedenti" },
      { code: "0010", label: "Festivit├á Soppresse" },
      { code: "BR", label: "Blocco Reperibilit├á" },
    ]
  },
  {
    group: "Congedi", emoji: "­ƒæÂ", color: "rose",
    items: [
      { code: "0112", label: "Congedo di Paternit├á" },
  ...
  {
    group: "Permessi", emoji: "­ƒôï", color: "blue",
    items: [
  ...
  {
    group: "Malattia e Salute", emoji: "­ƒÅÑ", color: "red",
    items: [
      ...
      { code: "0133", label: "Disponibilit├á Covid-19" },
    ]
  },
  {
    group: "Recupero Ore", emoji: "­ƒöä", color: "teal",
    items: [
  ...
  {
    group: "Straordinario", emoji: "ÔÅ░", color: "orange",
    items: [
  ...
  {
    group: "Formazione e Altro", emoji: "­ƒÄô", color: "indigo",
    items: [
...
export default function AgentDashboard({ currentUser, shifts, allAgents, currentYear, currentMonth, isPublished, currentView: _currentView, tenantSlug: _tenantSlug, canManageShifts, canManageUsers, canVerifyClockIns, canConfigureSystem, userRole, signOutAction }: AgentDashboardProps) {
  ...
  useEffect(() => {
    ...
    if (action) {
      if (action === 'sos') {
        const confirmSos = window.confirm("­ƒÜ¿ INVIARE SOS GPS ALLA CENTRALE? La tua posizione attuale verr├á trasmessa immediatamente.")
        if (confirmSos) {
          setTimeout(() => {
        ...
        toast("Visualizzazione turni mensili", { icon: "­ƒôà" })
      }
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        if (pos.coords.accuracy > 500) {
          ...
          return toast.error("Segnale GPS troppo debole! Esci all'aperto o attendi un segnale pi├╣ preciso.", { id: toastId })
        }
        
        if (pos.coords.accuracy > 100) {
          toast("Segnale GPS impreciso, la timbratura potrebbe essere rifiutata.", { icon: "ÔÜá´©Å" })
        }
        ...
        try {
          ...
          setIsClockedIn(type)
          setLastClockTime(new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }))
          toast.success(`ÔÜá´©Å Offline: Timbratura ${type === 'IN' ? 'Entrata' : 'Uscita'} archiviata localmente. Verr├á inviata appena torna il segnale.`, { id: toastId, duration: 5000 })
        } finally {
          setClockLoading(false)
        }
      },
      ...
      try {
        ...
        const res = await fetch('/api/admin/alert-emergency', {
          ...
          body: JSON.stringify({ 
            ...
            type: 'SOS',
            message: `­ƒåÿ SOS GPS! ${currentUser.name} (Matr. ${currentUser.matricola})`,
            note: sosNote,
            audio: audioBase64,
            lat: pos.coords.latitude,
            lng: pos.coords.longitude
        ...
        if (res.ok) {
          toast.success('­ƒÜ¿ SOS INVIATO CON SUCCESSO!', { id: toastId })
  ...
  const handleExportPDF = async () => {
    ...
    doc.text('Resoconto Mensile Attivit├á', 14, 22)
    ...
    autoTable(doc, {
      startY: 75,
      head: [['Categoria', 'Valore', 'Unit├á']],
      body: [
        ['Straordinario', straHours, 'ore'],
        ['Ferie / Festivit├á', ferieDays, 'giorni'],
        ['Recupero Ore', recHours, 'ore'],
        ['Reperibilit├á', repCount, 'turni'],
      ],
      theme: 'striped',
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] p-8 text-center animate-in fade-in zoom-in duration-700">
        ...
        <p className="text-slate-500 max-w-md mx-auto leading-relaxed mb-8">
          ...
          Riprova pi├╣ tardi per consultare la tua agenda definitiva.
        </p>
          ...
          <div className="p-6">
            ...
            <div className="mt-6 pt-6 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4">
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest max-w-sm text-center sm:text-left">
                Ôä╣´©Å Questa rubrica ├¿ visibile esclusivamente all&apos;Ufficiale incaricato per la giornata odierna.
              </p>
              <button 
                onClick={async () => {
                  if (!confirm('­ƒÜ¿ Inviare allerta emergenza via Telegram a tutto il team?')) return
                  const res = await fetch('/api/admin/alert-emergency', { method: 'POST' })
                  if (res.ok) toast.success('­ƒÜ¿ Allerta inviata con successo!')
          ...
          <div className="bg-gradient-to-r from-emerald-600 to-teal-700 rounded-3xl p-5 text-white shadow-xl shadow-emerald-500/20 relative overflow-hidden group">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 relative z-10">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-black text-lg uppercase leading-none mb-1">Sei in Servizio! ­ƒæ«ÔÇìÔÖé´©Å</h3>
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
          <div className="flex-1">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black tracking-tighter leading-tight mb-2">
              Ciao, {currentUser.name.split(" ")[0]}! ­ƒæï
            </h2>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
            <div className="bg-white/5 backdrop-blur-xl border border-white/10 px-5 py-3 rounded-2xl flex items-center gap-4">
              ...
              <div>
                <p className="text-[9px] text-emerald-300/60 font-black uppercase tracking-wider">Reperibilit├á</p>
                ...
                <div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mt-3">
                    <span className="text-[11px] font-bold text-slate-500 bg-slate-100 px-3 py-1 rounded-lg border border-slate-200">
                      ...
                      {myOds.shift.serviceType && ` ┬À ${myOds.shift.serviceType.name}`}
                    </span>
      ...
      <div className="bg-white rounded-[2rem] shadow-xl border border-slate-200 overflow-hidden">
        <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium tracking-tight">Focus {currentMonthName} {currentYear} ┬À {daysInMonth} Giorni</p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-x-auto custom-scrollbar">
          {!isPublished ? (
            <div className="py-12 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-sm text-slate-500 max-w-sm mt-2">I turni di reperibilit├á per questo mese non sono ancora stati consolidati e pubblicati dall&apos;Amministratore.</p>
              <PlanningMobileView 
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2 px-4 py-2 bg-slate-100 rounded-xl text-[10px] font-black text-slate-500 uppercase tracking-widest border border-slate-200">
                 Anno {currentYear} ┬À Contatori Attivi
              </div>
            </div>
          </div>
       ...
       <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
         <div className="lg:col-span-3 bg-white rounded-[2rem] shadow-xl border border-slate-200 overflow-hidden">
           <div className="p-6 border-b border-slate-100 bg-emerald-50/50 flex items-center gap-4">
             ...
             <div>
               <h3 className="font-black text-lg text-slate-900">Riepilogo Reperibilit├á</h3>
               <p className="text-xs text-slate-500 font-medium">{currentMonthName} {currentYear} ÔÇö {repCount} {repCount === 1 ? 'turno' : 'turni'} assegnati</p>
             </div>
           </div>
           ...
           <div className="p-6">
             {!isPublished ? (
               ...
               <div className="text-center py-8">
                 ...
                 <p className="text-sm font-semibold text-slate-500">Nessuna reperibilit├á assegnata per questo mese.</p>
               ...
               <div className="space-y-2">
                   ...
                   <div key={idx} className="flex items-center gap-4 bg-emerald-50/60 hover:bg-emerald-50 border border-emerald-100 rounded-xl px-5 py-3 transition-colors">
                     ...
                     <div className="flex-1">
                       <p className="text-sm font-bold text-slate-800">
                         Reperibilit├á <span className="text-emerald-600">{rd.repType}</span>
                       </p>
                       {rd.baseType && (
                 ...
                 <div className="flex items-center justify-between pt-4 mt-2 border-t border-slate-100">
                   <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Totale Reperibilit├á Mese</span>
             ...
             if (nextRepDate) {
               ...
               return (
                 <div className="bg-white rounded-[2rem] border border-slate-200 p-8 flex flex-col items-center text-center shadow-lg">
                   ...
                   <h4 className="font-black text-lg text-slate-900 mb-1">Prossima Reperibilit├á</h4>
                   ...
                   <p className="text-sm font-bold text-slate-500 mt-2">
                     {diffDays === 0 ? (
                       <span className="text-red-600 animate-pulse">­ƒö┤ OGGI!</span>
                     ) : diffDays === 1 ? (
                       <span className="text-amber-600">ÔÜá´©Å Domani</span>
                     ) : (
                       <>tra <span className="text-emerald-600">{diffDays}</span> giorni</>
                     )}
                   </p>
                   <div className="mt-4 pt-4 border-t border-slate-100 w-full text-[10px] font-bold text-slate-400 uppercase tracking-widest">Orario: 22:00 ÔÇô 07:00</div>
                 </div>
               )
             } else {
               return (
                 <div className="bg-white rounded-[2rem] border border-slate-200 p-8 flex flex-col items-center text-center shadow-lg">
                   ...
                   <h4 className="font-black text-lg text-slate-900 mb-1">Prossima Reperibilit├á</h4>
                   <p className="text-sm text-slate-400 font-medium mt-2">Nessuna reperibilit├á futura programmata.</p>
                 </div>
               )
             }
           })()}
             ...
             <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                     ...
                     if (isForbidden) {
                        return (
                          <div className="flex items-center gap-3 p-4 bg-rose-50 border border-rose-100 rounded-2xl text-rose-600">
                            ...
                            <p className="text-xs font-bold leading-tight uppercase tracking-tighter italic">
                              SCAMBIO NON POSSIBILE: Uno dei due operatori ├¿ a RIPOSO in questa data.
                            </p>
                          </div>
                        )
                     }
               ...
               <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4">
                 <p className="text-[9px] text-amber-700 font-black uppercase tracking-widest text-center leading-relaxed italic">
                   ÔÜá´©Å Premendo &quot;Invia Proposta&quot;, il sistema chieder├á prima l&apos;accordo al collega e poi il visto finale dell&apos;Admin o della Segreteria.
                 </p>
               </div>
             ...
             <div>
               ...
               <p className="text-blue-100/80 text-sm font-medium mt-1">
                 {balances?.user && balances.user.telegramChatId 
                   ? "Ô£à Il tuo account ├¿ collegato correttamente. Riceverai qui le allerte del Comando."
       ...
       <footer className="text-center py-6 border-t border-slate-100 mt-4">
         <p className="text-[10px] text-slate-400 font-medium uppercase tracking-widest">
           Comando di Polizia Locale di Altamura ┬À Sistema Reperibilit├á v1.0
         </p>
       </footer>
           ...
           <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden" onClick={e => e.stopPropagation()}>
             <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-5 text-white flex justify-between items-center">
               <div>
                 ...
                 <p className="text-blue-200 text-xs mt-0.5">Scegli il tuo dispositivo per importare tutte le reperibilit├á</p>
               </div>
             ...
             <div className="p-5 space-y-3">
               ...
               >
                 ...
                 <div>
                   ...
                   <p className="text-xs text-slate-500">Abbonamento automatico. Le reperibilit├á appariranno nel tuo Calendario Apple e si aggiorneranno in automatico.</p>
                 </div>
               </button>
               ...
               >
                 ...
                 <div>
                   ...
                   <p className="text-xs text-slate-500">Abbonamento a calendario Internet. Outlook importer├á tutti gli eventi e li terr├á aggiornati.</p>
                 </div>
               </button>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-5xl max-h-[92vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
              ...
              <div className="relative z-10 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-white/70 text-xs font-semibold mt-0.5">{currentMonthName} {currentYear} ┬À {currentUser.name}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
              <div className="w-full md:w-[380px] bg-slate-50 border-r border-slate-100 p-5 overflow-y-auto custom-scrollbar flex-shrink-0">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Ô£Å´©Å Nuovo Appunto</p>

                <div className="space-y-3">
                  <div>
                    <label className="block text-[10px] font-bold text-slate-600 uppercase tracking-widest mb-1 ml-0.5">­ƒôà Giorno</label>
                    <select 
                      value={agendaDate} 
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-slate-600 uppercase tracking-widest mb-1 ml-0.5">­ƒöì Cerca Tipo</label>
                    <div className="relative">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block text-[10px] font-bold text-slate-600 uppercase tracking-widest mb-1 ml-0.5">ÔÅ▒ Ore</label>
                      <input 
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-slate-600 uppercase tracking-widest mb-1 ml-0.5">­ƒôØ Note</label>
                      <input
                        type="text"
            ...
            <div className="p-6 sm:p-8 bg-slate-50">
               {balances && (
                 <div className="mb-6 grid grid-cols-3 gap-2">
                    ...
                    <div className="bg-white rounded-xl p-3 border border-slate-200 text-center shadow-sm">
                      <p className="text-[9px] font-black uppercase text-teal-500 tracking-wider">Festivit├á</p>
                  ...
                  <button 
                     ...
                     onClick={async () => {
                        ...
                        try {
                           ...
                           toast.success("Ô£à Richiesta inviata!");
