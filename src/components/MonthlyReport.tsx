...
export default function MonthlyReport() {
  ...
  const chartData = [...stats].sort((a, b) => b.oreLavorate - a.oreLavorate).slice(0, 10)
        ...
        <>
          ...
          {totals && (
            <div className="flex flex-wrap gap-4 bg-slate-50 border border-slate-200 rounded-xl p-4">
              <div className="flex items-center gap-2 text-xs font-bold text-slate-600"><span className="w-3 h-3 bg-blue-500 rounded-sm"></span> Ferie: {totals.totalFerie} gg</div>
          ...
          {chartData.length > 0 && (
            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
              <h3 className="text-sm font-bold text-slate-800 mb-4 flex items-center gap-2"><BarChart3 className="w-4 h-4 text-blue-500" /> Top 10 — Ore Lavorate</h3>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            <div className="p-5 border-b border-slate-100 flex items-center justify-between">
              <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2"><FileText className="w-4 h-4 text-indigo-500" /> Dettaglio per Agente</h3>
                  ...
                  {sortedStats.map((s, i) => (
                    <tr key={s.id} className={`border-t border-slate-50 hover:bg-blue-50/30 transition-colors ${i % 2 === 0 ? "" : "bg-slate-50/30"}`}>
                      <td className="p-3 font-bold text-slate-800 text-xs uppercase whitespace-nowrap">
                        ...
                        {s.name}{s.squadra && <span className="text-[9px] text-slate-400 ml-1.5">({s.squadra})</span>}
                      </td>
