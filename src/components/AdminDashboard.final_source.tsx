...
export default function AdminDashboard({ allAgents, shifts, currentYear, currentMonth, isPublished, currentView, settings, tenantSlug }: { 
  ...
  const openCellEditor = (agentId: string, agentName: string, day: number, currentType: string) => {
    ...
    let warningMsg = ""
    if (tType === "BR") {
      warningMsg = `BLOCCO REPERIBILIT├Ç ATTIVO: Non assegnare reperibilit├á in questo giorno.`
    } else if (tType && isBlocked(tType)) {
      warningMsg = `Attenzione: l'agente ha un'assenza registrata oggi (${tType}). La reperibilit├á NON dovrebbe essere assegnata.`
    } else if (nType === "BR") {
      warningMsg = `BLOCCO REPERIBILIT├Ç DOMANI: L'agente ├¿ bloccato il giorno successivo.`
    } else if (nType && isBlocked(nType)) {
      warningMsg = `Attenzione: l'agente ha un'assenza registrata domani (${nType}). La reperibilit├á odierna NON dovrebbe essere assegnata.`
    }
  ...
  const handleBulkSave = async () => {
    ...
    if (end < start) {
      toast.error("La data di fine non pu├▓ essere precedente all'inizio")
      return
    }

    if (!confirm(`Sei sicuro di voler inserire "${bulkData.code}" per il periodo selezionato? Sovrascriver├á i turni attuali.`)) return
  ...
  const handleRecalcAgent = async (agentId: string) => {
    if (!confirm("Ricalcolare le reperibilit├á di questo agente per questo mese? Le REP fisse non verranno toccate, ma i buchi verranno riempiti se necessario.")) return
  ...
  const generateVerbatelScript = async () => {
    ...
    if(!table) return alert('Tabella Verbatel non trovata! Assicurati di essere nella pagina corretta (Prospetto Reperibilit├á).');
  ...
  const handleClear = async (type: "all" | "base" | "rep") => {
    const messages = {
      all: "CANCELLARE TUTTI I DATI (Turni e Reperibilit├á)",
      base: "cancellare solo i TURNI BASE (M7, P14, F, ecc.)",
      rep: "cancellare solo le REPERIBILIT├Ç (R)"
    }
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: "base" | "rep") => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const detail = result.importType === "base" ? "Turni Base" : "Reperibilit├á"
        setUploadStatus(`Ô£à Importati ${result.count} ${detail} con successo!`)
        toast.success(`Importazione ${detail} completata: ${result.count} record.`)
        router.refresh()
        setTimeout(() => setUploadStatus(""), 5000)

      } catch (error) {
        setUploadStatus(`ÔØî ${error instanceof Error ? error.message : "Errore lettura Excel"}`)
      }
    }
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, `Reperibilit├á ${currentMonthName} ${currentYear}`)
  ...
  const handleExportUfficialiExcel = () => {
    const wsData = [
      ["PORTALE CASERMA - REPERIBILIT├Ç UFFICIALI", "", "", ""],
      [`Mese: ${currentMonthName} ${currentYear}`, "", "", ""],
      [],
  ...
  const handleExportRepPDF = async () => {
    setIsExportingPDF(true)
    const toastId = toast.loading("Generazione Prospetto REPERIBILIT├Ç...")
    try {
      ...
      console.error(error)
      toast.error("Errore export Reperibilit├á", { id: toastId })
    } finally {
      setIsExportingPDF(false)
    }
  }
  ...
  const handlePublish = async () => {
    ...
    setUploadStatus("ÔÜÖ´©Å Aggiornamento visibilit├á...")
    try {
      ...
      setUploadStatus("ÔØî Errore pubblicazione")
    } finally {
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex bg-red-50 rounded-lg p-1 border border-red-100">
            ...
            <button 
              ...
              title="Elimina solo le Reperibilit├á (R)"
            >
              <Trash2 width={14} height={14} />
              Reset REP
            </button>
          ...
          <button 
            ...
            onClick={async () => {
              if (!confirm(`Sei sicuro di voler INVIARE LA PEC a tutti i dipendenti anagrafati? Costituir├á valore di notifica ufficiale.`)) return
              
              setIsSendingPec(true)
              setUploadStatus("­ƒô¿ Invio PEC in corso...")
              try {
                ...
                setUploadStatus(`Ô£à PEC inviata con successo a ${data.emailSentCount} dipendenti!`)
              } catch (error) {
                setUploadStatus(`ÔØî ${error instanceof Error ? error.message : "Errore Invio PEC"}`)
              } finally {
            ...
            title="Invia il Riepilogo Reperibilit├á a tutti via Email Certificata"
          >
          ...
          <button 
            ...
            onClick={async () => {
              if (!confirm('­ƒÜ¿ SEI SICURO di voler lanciare un ALLERTA EMERGENZA via Telegram a tutti i reperibili di oggi?')) return
              setIsSendingAlert(true)
              try {
                const res = await fetch('/api/admin/alert-emergency', { method: 'POST' })
                const data = await res.json()
                if (!res.ok) throw new Error(data.error || 'Errore invio allerta')
                toast.success(`­ƒÜ¿ Allerta inviata a ${data.alerted} reperibili via Telegram!`)
              } catch (error) {
          ...
          <button 
            ...
            onClick={async () => {
              ...
              setUploadStatus("ÔÜÖ´©Å Algoritmo in esecuzione...")
              try {
                ...
                setUploadStatus(`Ô£à Generati ${result.totalAssigned} turni REP!${result.emptyDays.length > 0 ? ` ÔÜá´©Å Giorni scoperti: ${result.emptyDays.join(', ')}` : ''}`)
                router.refresh()
              } catch (error) {
                setUploadStatus(`ÔØî ${error instanceof Error ? error.message : "Errore"}`)
              } finally {
          ...
          >
            {isGenerating ? "ÔÜÖ´©Å Generazione in corso..." : (
              <>
                <Play width={18} height={18} fill="currentColor" />
                Genera Reperibilit├á
              </>
            )}
          </button>
          ...
          <button 
            ...
            onClick={async () => {
              if (!confirm('Vuoi attivare l\'Assistente AI per scansionare il mese e coprire i buchi di reperibilit├á?\n\nL\'algoritmo rispetter├á:\nÔÇó Stacco minimo notturno (no M il giorno dopo)\nÔÇó Massimale individuale\nÔÇó Assenze e malattie\nÔÇó Equit├á weekend (Sab/Dom)')) return
              setIsResolving(true)
              setUploadStatus("­ƒºÖÔÇìÔÖé´©Å Assistente AI in Calcolo...")
              try {
                ...
                setUploadStatus(`Ô£à ${data.holesResolved} buchi coperti in sicurezza!`)
                toast.success(`Coperti ${data.holesResolved} turni mancanti!`)
                router.refresh()
              } catch (error) {
                const msg = error instanceof Error ? error.message : 'Errore sconosciuto';
                setUploadStatus(`ÔØî Errore: ${msg}`)
        ...
        <div className="p-4 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-blue-50 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
          <div className="w-full lg:w-auto">
            ...
            <div className="flex items-center justify-between lg:justify-start w-full gap-4 mt-1">
              ...
              <p className="text-[10px] sm:text-xs text-slate-400 font-bold bg-slate-100 px-2 py-0.5 rounded-full">{allAgents.length} agenti ┬À {shifts.filter(s => (s.repType || s.type)?.toUpperCase().includes("REP")).length} REP</p>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full lg:w-auto overflow-hidden">
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto pb-1 -mb-1 scrollbar-hide">
              ...
              <button 
                ...
                disabled={isExportingPDF}
                title="Esporta solo Reperibilit├á"
                  ...
                  <tr>
                    ...
                    >
                      <div className="flex items-center justify-between px-1">
                        Agente
                        <span className="text-[10px] text-indigo-400">{sortConfig?.key === 'name' ? (sortConfig.dir === 'asc' ? 'Ôû▓' : 'Ôû╝') : 'Ôåò'}</span>
                      </div>
                    </th>
                    {dayInfo.map((di) => (
                      <th 
                    ...
                    <th className="px-2 pt-3 pb-1 text-center font-black bg-indigo-50 text-indigo-700 border-b-4 border-l-4 border-indigo-100 min-w-[40px] font-sans text-[9px] uppercase tracking-tighter sticky top-0 z-[50]" rowSpan={2} title="Reperibilit├á nei giorni Festivi e Weekend">
                      FEST
                    </th>
                    <th className="px-2 pt-3 pb-1 text-center font-black bg-slate-50 text-slate-600 border-b-4 border-slate-200 min-w-[40px] font-sans text-[9px] uppercase tracking-tighter sticky top-0 z-[50]" rowSpan={2} title="Reperibilit├á nei giorni Feriali">
                      FER
                    </th>
                    <th 
                    ...
                    >
                      <div className="flex flex-col items-center justify-center">
                        REP
                        <span className="text-[8px] opacity-60 mt-1">{sortConfig?.key === 'repTotal' ? (sortConfig.dir === 'asc' ? 'Ôû▓' : 'Ôû╝') : 'Ôåò'}</span>
                      </div>
                    </th>
                  </tr>
                  <tr>
                    ...
                    <>
                            ...
                            <td 
                              ...
                              title={
                                repDays.length > effectiveMassimale 
                                  ...
                                  : repDays.length > 0 
                                    ? `Giorni in reperibilit├á: ${repDays.join(', ')}\nFestivi: ${repFest}, Feriali: ${repFer}` 
                                    : "Nessuna reperibilit├á"
                              }
                            >
                              <div className="flex flex-col items-center">
                                <span className="flex items-center gap-1">
                                  {repDays.length > agent.massimale && "ÔÜá´©Å "}{repDays.length}
                                </span>
                        ...
                        {dayInfo.map(di => {
                          ...
                          return (
                            <td 
                              ...
                              title={namesList ? `Reperibili: ${namesList}` : "Nessuno in reperibilit├á"}
                      ...
                      <tr className="bg-slate-50/50 border-t border-slate-200">
                        ...
                        {dayInfo.map(di => {
                          ...
                          // BUG FIX: Usa allAgents invece di sortedAgents per mantenere visibilit├á globale anche durante la ricerca
            ...
            <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/80 shrink-0">
              <div className="flex items-center gap-4">
                 ...
                 <div>
                   ...
                   <p className="text-slate-500 text-[11px] font-bold uppercase tracking-widest mt-0.5">{editingCell.agentName} <span className="text-indigo-400 mx-1">ÔÇó</span> Giorno {editingCell.day}</p>
                 </div>
              </div>
              ...
              <div className="space-y-6">
                <div>
                  <div className="text-[10px] font-black text-slate-400 mb-3 flex items-center gap-2 uppercase tracking-widest">
                    <span>ÔÜí</span> OPERATIVI VELOCI
                  </div>
          ...
          <div className="relative w-full max-w-lg bg-white h-[100dvh] shadow-2xl flex flex-col animate-in slide-in-from-right duration-300 border-l border-slate-200">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-10 custom-scrollbar">
               ...
               <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 delay-100">
                  <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4 border-b border-slate-100 pb-2">1. Anagrafica e Identit├á</h4>
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-200 flex items-center gap-3 shrink-0 rounded-bl-3xl">
               <button onClick={async () => {
                  if (!confirm(`Vuoi davvero eliminare l&apos;operatore ${editingAgent.name}? L&apos;azione ├¿ irreversibile e distrugger├á tutto lo storico.`)) return
          ...
          <div className="relative bg-white w-full max-w-4xl max-h-[90vh] rounded-[3rem] shadow-2xl overflow-hidden flex flex-col border border-white animate-in zoom-in-95 duration-300">
            ...
            <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-white shrink-0">
              <div className="flex items-center gap-5">
                ...
                <div>
                  <h2 className="text-2xl font-black text-slate-900 tracking-tight uppercase">Registro Attivit├á</h2>
                  <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.2em] mt-1">Sentinel Autonomous Audit System ÔÇö Ultime 100 Azioni</p>
                </div>
              </div>
              <button 
          ...
          <div className="relative bg-white w-full max-w-4xl max-h-[90vh] rounded-[3rem] shadow-2xl overflow-hidden flex flex-col border border-white animate-in zoom-in-95 duration-300">
              ...
              <div className="flex items-center gap-5">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                     ...
                     <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest">ÔÇó</span>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8 bg-slate-50/30 custom-scrollbar">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                 {[
                   { step: "01", title: "Aperto Verbatel", desc: "Seleziona 'Prospetto Reperibilit├á'" },
                              ...
                              <div className="flex items-center gap-6">
                                 ...
                                 <div>
                                    ...
                                    <div className="flex flex-wrap items-center gap-3">
                                       ...
                                       {req.startTime && <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1"><RefreshCw width={10}/> {req.startTime} ÔÇó {req.endTime}</span>}
                                    </div>
                           ...
                           <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 relative z-10">
                              <div className="flex flex-col sm:flex-row items-center gap-6">
                                 ...
                                 <div className="text-center sm:text-left">
                                    ...
                                    <h5 className="font-black text-slate-900 text-sm uppercase"><span className="text-slate-500">{swap.requester.name}</span> <span className="text-indigo-600">ÔåÆ</span> {swap.targetUser.name}</h5>
            ...
            <div className="p-6 bg-slate-900 text-white shrink-0">
               <div className="flex items-start gap-4">
                  ...
                  <p className="text-[10px] text-white/60 font-medium leading-relaxed">
                    ATTENZIONE: L&apos;approvazione ├¿ definitiva e sposter├á i turni nel Database centrale, notificando immediatamente gli interessati via Telegram e Push.
                  </p>
               </div>
            </div>
          </div>
            ...
            <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-teal-50/30 shrink-0">
              <div className="flex items-center gap-5">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-teal-600 font-black uppercase tracking-[0.2em] mt-1">Sentinel Bulk Entry System ÔÇö Inserimento Massivo</p>
                </div>
              </div>
              <button 
          ...
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col overflow-hidden border border-white/20" onClick={e => e.stopPropagation()}>
                   ...
                   {activeDetailTab === 'SALDI' && (
                     <div className="space-y-8">
                        ...
                        <div className="bg-amber-50 h-2 border-2 border-amber-100 border-dashed rounded-full px-10 py-6 flex items-center gap-4">
                           ...
                           <p className="text-xs font-bold text-amber-800">I saldi vengono calcolati sottraendo sia i giorni gi├á effettuati che le richieste in attesa di approvazione.</p>
                        </div>
                     </div>
                   )}
                        ...
                        <div className="grid grid-cols-1 gap-4">
                           ...
                           {shifts.filter((s) => s.userId === selectedAgentForDetails.id).length === 0 && (
                              <div className="text-center py-20 bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200">
                                 <p className="text-slate-400 font-bold italic text-sm">Nessuna attivit├á registrata per questo mese.</p>
                              </div>
                           )}
                        </div>
                     </div>
             ...
             <div className="bg-white px-12 py-8 border-t-2 border-slate-50 flex justify-between items-center shrink-0">
                <div className="flex items-center gap-4">
                   ...
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Sistema Gestionale Caserma ┬À Profilo {selectedAgentForDetails.isUfficiale ? 'Ufficiale' : 'Agente'}</p>
                </div>
                <button 
                  onClick={() => {
