...
# The New Premium UI Content
premium_ui = """      {/* === ANAGRAFICA MODAL === */}
      {showAnagrafica && (
...
for i, line in enumerate(lines):
    if "{/* === ANAGRAFICA MODAL === */}" in line:
        start_idx = i
    if "{/* Area anagrafica nascosta qui... rimossa per spazio modal */}" in line:
        end_idx = i
        break

if start_idx != -1 and end_idx != -1:
    new_content = lines[:start_idx] + [premium_ui + "\\n"] + lines[end_idx:]
    with open(file_path, "w", encoding="utf-8") as f:
        f.writelines(new_content)
    print("SUCCESS: AdminDashboard repaired with Premium UI.")
else:
    print(f"ERROR: Markers not found. start={start_idx}, end={end_idx}")
    # Fallback to line numbers if markers fail due to corruption
    # This is a bit more dangerous but necessary if corruption is severe
    if start_idx == -1: # Only try fallback if start marker is missing
        print("Attempting fallback line numbers...")
        # Based on my latest view_file, 1554 was the start.
        # But we use 0-indexing for lines[]
        fallback_start = 1553 
        fallback_end = 1746 
        new_content = lines[:fallback_start] + [premium_ui + "\\n"] + lines[fallback_end:]
        with open(file_path, "w", encoding="utf-8") as f:
            f.writelines(new_content)
        print("SUCCESS: Applied repair via fallback line numbers.")
